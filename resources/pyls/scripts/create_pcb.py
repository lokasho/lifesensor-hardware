...
def create_pcb(settings: create_utils.Settings):
    ...
    create_utils.update_file_name_and_content(
        create_utils.board_dir(settings), settings
    )
...
def ensure_component_dir_exists(settings: create_utils.Settings) -> None:
    ...
    if not comp_dir.exists() and input(create_question).lower().startswith("y"):
        ...
        comp_settings = create_utils.Settings(
            settings.repo, settings.component, settings.variant
        )
