...
def pcbnew_path():
    # not running as plugin, therefor the pcbnew library is searched manually for
    if sys.platform.startswith("win32"):
        # TODO: get windows path for pcbnew lib
        raise NotImplemented("I do not know how to import the pcbnew library")
    elif sys.platform.startswith("linux"):
        # TODO: get linux path for pcbnew lib
        raise NotImplemented("I do not know how to import the pcbnew library")
    elif sys.platform.startswith("darwin"):
        pcbnew_path = "/Applications/KiCad/kicad.app/Contents/Frameworks/python/site-packages/pcbnew.py"
    return pcbnew_path


def import_pcbnew_from_path(path: str):
    if sys.version.startswith("2"):
        import imp
        pcbnew = imp.load_source("pcbnew", path)
    elif sys.version.startswith("3.3") or sys.version.startswith("3.4"):
        from importlib.machinery import SourceFileLoader
        pcbnew = SourceFileLoader("pcbnew", path).load_module()
    else:
        import importlib.util
        spec = importlib.util.spec_from_file_location("pcbnew", path)
        pcbnew = importlib.util.module_from_spec(spec)
    return pcbnew
...
try:
    ...
    import importlib
    pcbnew = import_pcbnew_from_path(pcbnew_path())
...
#     sheet_indices = [m.start() for m in re.finditer(r'\$Sheet', file_lines)]
#     endsheet_indices = [m.start() for m in re.finditer(r'\$EndSheet', file_lines)]
