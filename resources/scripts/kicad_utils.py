...
kicad_config_dir = dict(
    darwin=pathlib.Path.home() / "Library/Preferences/kicad/"
...
def extract_subsheets(file_path: pathlib.Path) -> Sequence[pathlib.Path]:
    ...
    assert len(sheet_line_indices) == len(endsheet_line_indices), \
...
def walk_all_sch_lines(project_file_path: pathlib.Path, file_idx_linehandler):
    ...
    all_sch_files = find_all_sch_files(str(main_schematic_file_path))
...
def available_libraries(project_dir: pathlib.Path):
    ...
    # load project symbol library table
...
def resolved_env_vars(path: (str, pathlib.Path)) -> pathlib.Path:
    path = str(path)
    if "${" in path:
...
def idcs_lines_starting_with(lines, start):
    matches = tuple(
...
class KiCadProject:
    ...
    def __init__(self, path: pathlib.Path):
        ...
        self._schematics = tuple(
            Schematic(self, p) for p in sch_paths)
        self._layout = Layout(self.path.with_suffix(".kicad_pcb"))
        self._symbol_cache = Library(self.name + "-cache", self.basedir / (self.name + "-cache.lib"))
        self._loaded_libs = {}
        ...
        self._sym_lib_table = SymLibTable(self._table_path) if self._table_path.exists() else None
    ...
    def archive_symbols(self, conflict_solver: CopyConflictSolver, dry_run=True):
        ...
        for ref in unarchived_refs:
            ...
            new_sym = self._handled_symbols.get(ref.library_nick, {}).get(ref.name, None)
            if not new_sym:
                source_lib = self.load_library(ref.library_nick)
                new_sym = copy_symbol(ref.name, source_lib, self._archive, conflict_solver)
    ...
    def load_library(self, nick: str) -> "Library":
        if nick in self._loaded_libs:
            ...
            lib_path = str(lib_path).replace("${KIPRJMOD}", str(self.basedir))
            lib = Library(nick, pathlib.Path(lib_path))
...
def copy_symbol(
    ...
    source_lib: "Library",
    dest_lib: "Library",
    conflict_solver: CopyConflictSolver) -> "Symbol":
    logger.info("Copying {} from {} to {}.".format(name, source_lib.nickname, dest_lib.nickname))
    if name in dest_lib.symbols:
        ...
        if source_content != dest_content:
            ...
            if solution is CopyConflictSolution.CREATE_NEW:
                ...
                raise ValueError("The conflict solving function, did not return a valid solution.")
        else:
            # symbols are equal, nothing to be done
            logger.info("Symbol {} already available in {}".format(name, dest_lib.nickname))
            dest_name = name
    else:
        # copy symbol to dest
        dest_name = name
...
SymbolReference = namedtuple(
    "SymbolReference",
    ("schematic", "line_idx", "library_nick", "name"))
...
class SymLibTable:
    ...
    def __init__(self, path: pathlib.Path):
        self._path = path
        self._lines = list(self.EMPTY_TEMPLATE)  # type: Sequence[str]
        self._libs = {}
...
class Library:
    ...
    EMPTY_TEMPLATE = ("EESchema-LIBRARY Version 2.4\n", "#encoding utf-8\n", "\n", "#End Library\n")
    ...
    def _parse_file(self) -> None:
        ...
        for idx, line in enumerate(self._lines):
            if line.startswith("DEF"):
                assert state is OUTSIDE_SYMBOL
                state = WITHIN_SYMBOL
                curr_first_line_idx = idx - len(curr_content)
                assert 0 <= curr_first_line_idx < len(self._lines)
                curr_content.append(line)
                # the symbols name is always given after the "DEF"
                curr_name = line.split()[1]
            elif line.startswith("ENDDEF"):
                assert state is WITHIN_SYMBOL
                state = OUTSIDE_SYMBOL
                curr_content.append(line)
                curr_last_line_idx = curr_first_line_idx + len(curr_content)
                assert 0 < curr_last_line_idx < len(self._lines)
                new_sym = Symbol(
                    curr_name,
                    (curr_first_line_idx, curr_last_line_idx),
                    curr_content)
                self._symbols[curr_name] = new_sym
                for alias in curr_aliases:
                    self._symbols[alias] = new_sym
                if curr_aliases:
                    logger.debug("Loaded symbol '{}:{} ({})'".format(self.nickname, new_sym.name, curr_aliases))
                else:
                    logger.debug("Loaded symbol '{}:{}'".format(self.nickname, new_sym.name))
                curr_content = list()
                curr_aliases = list()
            elif state == WITHIN_SYMBOL:
                if line.startswith("ALIAS"):
                    alias = line.strip().split()[1]
                    curr_aliases.append(alias)
                curr_content.append(line)
            elif state == OUTSIDE_SYMBOL and line.startswith("#"):
                curr_content.append(line)
            else:
                curr_content.clear()
...
if __name__ == '__main__':
    path = pathlib.Path("../../components/ecg_ads1292/pcbs/ecg_ads1292_main/kicad/ecg_ads1292_main.pro")
    project = KiCadProject(path)

    def solve_conflict(name, lib_a, lib_b):
        logger.info("Conflict for {}, between library {} and {}".format(name, lib_a.path, lib_b.path))
